...
function ArrayVisualizer() {
  ...
  useEffect(() => {
    ...
    const arrayNameWidth = 120; // Width reserved for array name
    ...
    const elementGroups = arrayGroup.selectAll(".array-element")
      .data(currentState.content, (d, i) => `element-${i}-${d}`);
    
    // Remove old elements with exit animation
    elementGroups.exit()
      .transition()
      .duration(500)
      .attr("transform", (d, i) => `translate(${i * (cellSize + 5)}, ${height})`)
      .remove();
    ...
    const enterGroups = elementGroups.enter()
      ...
      .attr("transform", (d, i) => `translate(${i * (cellSize + 5)}, -60)`)
      .style("opacity", 0);
    ...
    enterGroups.append("rect")
      ...
      .attr("ry", 4)
      .attr("class", (d, i) => {
        const actualIndex = currentState.content.indexOf(d);
        if (currentState.changes.added.includes(actualIndex)) return "element-added";
        if (currentState.changes.modified.includes(actualIndex)) return "element-modified";
    ...
    enterGroups.append("text")
      ...
      .attr("text-anchor", "middle")
      .text((d, i) => {
        // Find the actual index in the current array
        return currentState.content.indexOf(d);
      });
    ...
    enterGroups.transition()
      .duration(600)
      .attr("transform", (d, i) => {
        // Get the actual index in the current array
        const actualIndex = currentState.content.indexOf(d);
        return `translate(${actualIndex * (cellSize + 5)}, 0)`;
      })
      .style("opacity", 1);
  ...
  useEffect(() => {
    ...
    const labelWidth = 90; // Narrower label area
